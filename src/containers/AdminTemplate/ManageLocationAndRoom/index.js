  ...
  const renderListLocations = () => {
    return listLocation?.map((id) => {
      return (
        <tr key={id._id}>
          <th>
            <Link to={`/list-rooms/${id._id}`}>{id.name}</Link>
          </th>
          <th>{id.province}</th>
          ...
          <th className="d-none d-md-table-cell">
            <BorderColorOutlinedIcon
              ...
              onClick={() => history.push(`edit-location/${id._id}`)}
            />
            <DeleteSweepOutlinedIcon
              className="button-delete"
              onClick={() => dispatch(actDeleteLocationApi(id._id))}
            />
          </th>
        </tr>
      );
    ...
    <Container maxWidth="md" sx={{ mt: 10 }}>
      ...
      <Root sx={{ width: 500, maxWidth: "100%" }}>
        <table aria-label="custom pagination table" className="table">
          ...
          <tbody>{renderListLocations()}</tbody>
        </table>
      </Root>
    </Container>
  );
